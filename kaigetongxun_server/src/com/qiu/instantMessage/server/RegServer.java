...
import com.qiu.instantMessage.db.UsernameException;
	...
	public void run() {
		...
		try {
			...
			if (type.equals("code")) {// 验证码的请求
				...
				if (code1 != null) {
					hashMap_code.remove(username);// 一定要删除验证码 不管正确与否
				}
				...
				if (code1.equals(code)) {// 询问验证码是否正确
					...
					try {
						...
						output.write("{\"state\":1,\"msg\":\"用户名已存在!\"}".getBytes());
						output.flush();
						return;
					...
					output.write("{\"state\":2,\"msg\":\"验证码错误，请重新获得!\"}".getBytes());
					output.flush();
				}
